    ...
    for llX in np.arange(minX, maxX, clipSizeMX):
        for llY in np.arange(minY, maxY, clipSizeMY):
            ...
            if (polyCut).Intersection(geom):
                ...
                maxXCut = envCut[1]
                maxYCut = envCut[3]
                outputFileName = os.path.join(outputDirectory, outputPrefix + rasterFileBase.replace('.tif',
                                                                                                     "_{}_{}.tif".format(
                                                                                                         minXCut,
                                                                                                         minYCut)))
                ## Clip Image
...
if __name__ == '__main__':
    ...
    outlineSrc = '/usr/local/share/spacenet/Rio_Buildings_West_AOI_geojson/Outline/AOI_EAST_OUTLINE.geojson'
    buildingsSrc = '/usr/local/share/spacenet/Rio_Buildings_West_AOI_geojson/buildigns/AOIEast_V4.geojson'
    outdir = '/usr/local/share/spacenet/combinedClipTest/'

    # Read in the AOI outline file
    outline = ogr.Open(outlineSrc)
    layer = outline.GetLayer()
    for image in rastList:
        rasterFile = '/usr/local/share/spacenet/mosaic/' + image
        print rasterFile
    ...
    outlineSrc = '/usr/local/share/spacenet/Rio_Buildings_West_AOI_geojson/Outline/AOI_EAST_OUTLINE.geojson'
    buildingsSrc = '/usr/local/share/spacenet/Rio_Buildings_West_AOI_geojson/Rio_West_AOI_Outline.geojson'
